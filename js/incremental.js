...
function InitBuildings() {
	LoadBuilding("Minimum Wage Job",25,5,0.2);
	LoadBuilding("Shift Leader",100,0,0.5);
...
function LoadBuilding(name,cost,persec,inf) {
	var digit = buildings.length;
	
	buildings[digit] = new Building(name,cost,persec,inf);
	buildings[digit].Name = name;
	buildings[digit].Cost = cost;
	buildings[digit].PerSec = persec;
	buildings[digit].Inf = inf;
...
function Tick() {
	...
	if (buildings[0].Qty != 1) {
		if (game.money >= buildings[0].Cost) {
			document.getElementById("minwagjob").disabled = false;
		}
	} else {
			document.getElementById("minwagjob").disabled = true;
			document.getElementById("Building0.Cost").innerHTML = "Bought";
	}
}
...
function MinWagJob() {
	if (buildings[0].Qty == 0) {
		if (game.money >= buildings[0].Cost) {
			game.money -= buildings[0].Cost;
			buildings[0].Qty = buildings[0].Qty + 1;
			document.getElementById("money").innerHTML = game.money;
				var temp = 0;
				for (var i = 0; i < buildings.length; i++){
					temp += buildings[i].PerSec * buildings[i].Qty
			...
			document.getElementById("moneyPerSec").innerHTML = temp;
				var temp2 = 0.0;
				for (var i = 0; i < buildings.length; i++){
					temp2 += buildings[i].Inf
				}
			document.getElementById("influencePerSec").innerHTML = buildings[0].Inf;;
		}
				document.getElementById("Building0.Persec").innerHTML = buildings[0].PerSec
	}
}
